...
namespace CleverbotIO.Net
{
    ...
    public class CleverbotSession
    {
        ...
        public static async Task<CleverbotSession> NewSessionAsync(string apiUser, string apiKey)
        {
            ...
            var createResponse = await CleverbotCreateApi.GetResponseAsync(new CleverbotCreateRequest(session.ApiUser, session.ApiKey));

            if (createResponse.Status != "success")
	            throw new Exception($"The bot creation wasn't successful. Check your API credentials and try again. API returned status: {createResponse.Status}");
		...
		public static CleverbotSession NewSession(string apiUser, string apiKey)
			=> NewSessionAsync(apiUser, apiKey).Result;
        ...
        {
            var askResponse = await CleverbotAskApi.GetResponseAsync(new CleverbotAskRequest(ApiUser, ApiKey, BotNick, message));
